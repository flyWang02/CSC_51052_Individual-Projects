...
const EnergySankey = ({ selectedYear, countryData }) => {
  ...
  useEffect(() => {
    ...
    links.forEach((link, i) => {
      ...
      // Use value-based color for the gradient
      const startColor = linkColorScale(link.value);
      const endColor = linkColorScale(link.value * 10); // Slightly darker for target
    ...
    svg.append("g")
      ...
      .attr("fill", "#333")
      .text(d => `${d.name} (${d.value.toFixed(1)} ktoe)`);
